...
plt.ion()
fig = plt.figure()
fig.canvas.manager.window.wm_geometry("+100+200")
ax = fig.add_subplot(111, projection='3d')

# Initialize line handles (empty)
hand_line, = ax.plot([], [], [], label="Hand", color="blue")
robot_line, = ax.plot([], [], [], label="Robot", color="red")
#
# ax.set_xlim([-1, 1])
# ax.set_ylim([-1, 1])
# ax.set_zlim([-1, 1])
...
def compute_derivatives(position_list, time_step=0.033):  # ~30 FPS, adjust as needed
    velocities = []
    accelerations = []

    # Compute velocity: (x_t - x_{t-1}) / dt
    for i in range(1, len(position_list)):
        vel = (np.array(position_list[i]) - np.array(position_list[i-1])) / time_step
        velocities.append(vel)

    # Compute acceleration: (v_t - v_{t-1}) / dt
    for i in range(1, len(velocities)):
        acc = (velocities[i] - velocities[i-1]) / time_step
...
while True:
    ...
    if paused and was_paused is False:
        ...
        if robot_positions:
            ...
            hand_df = pd.DataFrame(hand_positions, columns=['x', 'y', 'z'])
            robot_df = pd.DataFrame(robot_positions, columns=['x', 'y', 'z'])
            latency_df = pd.DataFrame(latency_records, columns=['Gesture to command', 'Command to motion', 'Gesture to motion'])

            # Calculate velocity and acceleration
            hand_velocities, hand_accelerations = compute_derivatives(hand_positions)
            robot_velocities, robot_accelerations = compute_derivatives(robot_positions)

            # Add timestamps assuming 30 FPS (adjust if needed)
            time_step = 0.033  # seconds between frames
            time_vals_v = [i * time_step for i in range(len(hand_velocities))]
            time_vals_a = [i * time_step for i in range(len(hand_accelerations))]

            # Create DataFrames
            hand_vel_df = pd.DataFrame(hand_velocities, columns=['vx', 'vy', 'vz'])
            hand_vel_df.insert(0, 'time', time_vals_v)
            robot_vel_df = pd.DataFrame(robot_velocities, columns=['vx', 'vy', 'vz'])
            robot_vel_df.insert(0, 'time', time_vals_v)

            hand_acc_df = pd.DataFrame(hand_accelerations, columns=['ax', 'ay', 'az'])
            hand_acc_df.insert(0, 'time', time_vals_a)
            robot_acc_df = pd.DataFrame(robot_accelerations, columns=['ax', 'ay', 'az'])
            robot_acc_df.insert(0, 'time', time_vals_a)

            # Set your target folder
            folder_path = r"C:\Users\Lydia\Undergraduate Research w Dr. Korivand\trials\velocity and accel data"
            os.makedirs(folder_path, exist_ok=True)  # Ensure folder exists

            timestamp = time.strftime("%Y-%m-%d_%H-%M-%S")
            filename = os.path.join(folder_path, f"3D Latency_w_trajectory_{timestamp}.xlsx")

            with pd.ExcelWriter(filename, engine='openpyxl') as writer:
                hand_df.to_excel(writer, sheet_name='Hand Trajectory', index=False)
                robot_df.to_excel(writer, sheet_name='Robot Trajectory', index=False)
                latency_df.to_excel(writer, sheet_name='Latency Summary', index=False)

                hand_vel_df.to_excel(writer, sheet_name='Hand Velocity', index=False)
                robot_vel_df.to_excel(writer, sheet_name='Robot Velocity', index=False)
                hand_acc_df.to_excel(writer, sheet_name='Hand Acceleration', index=False)
                robot_acc_df.to_excel(writer, sheet_name='Robot Acceleration', index=False)

            print(f"✅ Saved: {filename}")
        else:
            print("⚠️ No data collected — skipping save.")
    ...
    if updated:
        try:
            ...
            #     hand_arr = np.array(hand_positions)
            #     hand_line.set_data(hand_arr[:, 0], hand_arr[:, 1])
            #     hand_line.set_3d_properties(hand_arr[:, 2])
            #
            # if len(robot_positions) > 1:
            #     robot_arr = np.array(robot_positions)
            #     robot_line.set_data(robot_arr[:, 0], robot_arr[:, 1])
            #     robot_line.set_3d_properties(robot_arr[:, 2])
            #
            # plt.draw()
            # plt.pause(0.001)  # brief pause to refresh
    ...
    if paused:
        ...
        if len(hand_positions) > 1 and len(robot_positions) > 1:
            plt.ioff()
            plt.show()
            ...
            ax = fig.add_subplot(111, projection='3d')
            ax.plot(hand_arr[:, 0], hand_arr[:, 1], hand_arr[:, 2], label='Hand Motion', color='blue')
            ax.plot(robot_arr[:, 0], robot_arr[:, 1], robot_arr[:, 2], label='Robot Motion', color='red')
            ax.set_title("Hand vs Robot Trajectory")
            ax.set_xlabel("X")
            ax.set_ylabel("Y")
            ax.set_zlabel("Z")
            ax.legend()
            ...
            time_vals_v = np.linspace(0, len(hand_velocities) * 0.033, len(hand_velocities))  # assuming 30Hz
            time_vals_a = np.linspace(0, len(hand_accelerations) * 0.033, len(hand_accelerations))

            # Plot velocity magnitudes
            fig, ax = plt.subplots(figsize=(10, 5))
            ax.plot(time_vals_v, [np.linalg.norm(v) for v in hand_velocities], label='Hand Velocity', color='blue')
            ax.plot(time_vals_v, [np.linalg.norm(v) for v in robot_velocities], label='Robot Velocity', color='red')
            ...
            fig, ax = plt.subplots(figsize=(10, 5))
            ax.plot(time_vals_a, [np.linalg.norm(a) for a in hand_accelerations], label='Hand Acceleration',
                    color='blue')
            ax.plot(time_vals_a, [np.linalg.norm(a) for a in robot_accelerations], label='Robot Acceleration',
                    color='red')
            ...
            hand_positions.clear()
            robot_positions.clear()
            hand_velocities.clear()
            robot_velocities.clear()
            hand_accelerations.clear()
            robot_accelerations.clear()
    ...
    actual_robot_pose = robot.get_actual_tcp_pose()
