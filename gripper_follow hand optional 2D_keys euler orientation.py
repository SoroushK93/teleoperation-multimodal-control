...
controls_text = [
    ...
    ["⌨Spacebar: Pause/Resume", "👆First Finger(R): Slowest", "👍Thumbs Up (R): Go Up"],
...
while True:
    ...
    ROTATION_STEP = 0.02  # ~1 degree per frame

    if keyboard.is_pressed('w'):  # pitch up
        pose[3:6] = (R.from_rotvec(pose[3:6]) * R.from_euler('x', ROTATION_STEP)).as_rotvec()
        updated = True
        updated_rotation = True
    elif keyboard.is_pressed('s'):  # pitch down
        pose[3:6] = (R.from_rotvec(pose[3:6]) * R.from_euler('x', -ROTATION_STEP)).as_rotvec()
        updated = True
        updated_rotation = True
    elif keyboard.is_pressed('a'):  # yaw left
        pose[3:6] = (R.from_rotvec(pose[3:6]) * R.from_euler('y', ROTATION_STEP)).as_rotvec()
        updated = True
        updated_rotation = True
    elif keyboard.is_pressed('d'):  # yaw right
        pose[3:6] = (R.from_rotvec(pose[3:6]) * R.from_euler('y', -ROTATION_STEP)).as_rotvec()
        updated = True
        updated_rotation = True
    elif keyboard.is_pressed('q'):  # roll CCW
        pose[3:6] = (R.from_rotvec(pose[3:6]) * R.from_euler('z', ROTATION_STEP)).as_rotvec()
        updated = True
        updated_rotation = True
    elif keyboard.is_pressed('e'):  # roll CW
        pose[3:6] = (R.from_rotvec(pose[3:6]) * R.from_euler('z', -ROTATION_STEP)).as_rotvec()
        updated = True
        updated_rotation = True
...
df.to_csv("euler_orientation_quaternions.csv", index=False)
print("Saved orientation data to 'euler_orientation_quaternions.csv'")
